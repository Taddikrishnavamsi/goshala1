...
const generateStars = (rating) => {
    let starsHTML = '';
    // Fix: Round rating to nearest integer to avoid fractional star issues
    const roundedRating = Math.round(rating);
    for (let i = 1; i <= 5; i++) starsHTML += `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-star ${i <= roundedRating ? 'fill-yellow-400 stroke-yellow-500 text-yellow-500' : 'fill-stone-200 stroke-stone-300 text-stone-300'}"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>`;
    return starsHTML;
};
